    ...
    def __init__(self, value=0, min_val=0, max_val=9999, step=1):
        ...
        btn_style = """
            QPushButton {
                ...
                border: 1px solid #E5E6EB; border-radius: 6px; font-weight: bold; font-size: 16px;
            }
            QPushButton:hover { background-color: #E5E6EB; border-color: #C9CDD4; color: #1D2129; }
        ...
        self.line_edit.setStyleSheet("""
            QLineEdit {
                background-color: #FFFFFF; border: 1px solid #E5E6EB;
                border-radius: 6px; padding: 0; font-weight: bold; color: #1D2129;
            }
            QLineEdit:focus { border: 2px solid #165DFF; }
        """)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("批量文件管理专家 V5.0 (现代光感重构版)")
    ...
    def apply_modern_theme(self):
        # 彻底优化的现代高级UI配色方案，功能区隔更加分明，兼容修复深色模式弹窗变黑Bug
        stylesheet = """
        /* 强制主窗口、对话框和弹窗拥有亮色背景，防止系统深色模式污染导致文字看不见 */
        QMainWindow, QDialog, QMessageBox { background-color: #F2F3F5; }
        QWidget { color: #1D2129; font-family: "Segoe UI", "Microsoft YaHei", sans-serif; font-size: 13px; }
        
        /* 针对弹窗 QMessageBox 特别适配，解决黑底黑字问题 */
        QMessageBox { background-color: #FFFFFF; border-radius: 8px; }
        QMessageBox QLabel { color: #1D2129; font-size: 14px; font-weight: 500; background: transparent; }
        QMessageBox QPushButton { 
            background-color: #165DFF; color: #FFFFFF; border: none; 
            border-radius: 6px; padding: 8px 20px; font-weight: bold; min-width: 70px; min-height: 20px;
        }
        QMessageBox QPushButton:hover { background-color: #4080FF; }
        QMessageBox QPushButton:pressed { background-color: #0E42D2; }
        
        /* 全局输入框 */
        QLineEdit, QComboBox, QTextEdit { 
            background-color: #FFFFFF; border: 1px solid #E5E6EB; border-radius: 6px; padding: 8px 12px; color: #1D2129; 
        }
        QLineEdit:hover, QComboBox:hover { border: 1px solid #C9CDD4; }
        QLineEdit:focus, QComboBox:focus, QTextEdit:focus { border: 2px solid #165DFF; background-color: #FFFFFF; }
        
        /* 导航芯片按钮 */
        QPushButton[cssClass="nav-chip"] { 
            background-color: #F2F3F5; color: #4E5969; border: 1px solid transparent; border-radius: 10px; padding: 10px 14px; font-weight: bold; font-size: 13px; 
        }
        QPushButton[cssClass="nav-chip"]:hover { background-color: #E5E6EB; color: #1D2129; }
        QPushButton[cssClass="nav-chip"]:checked { background-color: #E8F3FF; color: #165DFF; border: 1px solid #165DFF; }
        
        /* 顶部与功能操作按钮 */
        QPushButton[cssClass="btn-primary"] { background-color: #165DFF; color: #FFFFFF; border: none; border-radius: 6px; padding: 8px 16px; font-weight: bold; }
        QPushButton[cssClass="btn-primary"]:hover { background-color: #4080FF; }
        
        QPushButton[cssClass="btn-success"] { background-color: #00B42A; color: #FFFFFF; border: none; border-radius: 6px; padding: 8px 16px; font-weight: bold; }
        QPushButton[cssClass="btn-success"]:hover { background-color: #23C343; }
        
        QPushButton[cssClass="btn-secondary"] { background-color: #FFFFFF; color: #4E5969; border: 1px solid #E5E6EB; border-radius: 6px; padding: 8px 16px; font-weight: bold; }
        QPushButton[cssClass="btn-secondary"]:hover { background-color: #F2F3F5; border-color: #C9CDD4; color: #1D2129; }
        
        QPushButton[cssClass="btn-danger"] { background-color: #FFECE8; color: #F53F3F; border: 1px solid #FDCEC5; border-radius: 6px; padding: 8px 16px; font-weight: bold; }
        QPushButton[cssClass="btn-danger"]:hover { background-color: #F53F3F; color: #FFFFFF; border-color: #F53F3F; }
        
        /* 底部巨大的主执行按钮 */
        QPushButton[cssClass="btn-execute"] { 
            background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #165DFF, stop:1 #4080FF);
            color: #FFFFFF; border: none; font-size: 16px; border-radius: 12px; font-weight: bold; 
        }
        QPushButton[cssClass="btn-execute"]:hover { background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #4080FF, stop:1 #6AA1FF); }
        QPushButton[cssClass="btn-execute"]:disabled { background: #C9CDD4; color: #FFFFFF; }
        
        /* 阴影白底卡片面板 */
        QFrame#CardPanel { background-color: #FFFFFF; border-radius: 14px; border: 1px solid #E5E6EB; }
        
        /* === 设置项群组 (强化视觉分割) === */
        QGroupBox { 
            border: 1px solid #E5E6EB; border-radius: 8px; margin-top: 18px; 
            padding: 20px 15px 15px 15px; background-color: #FAFAFB; 
        }
        QGroupBox::title { 
            subcontrol-origin: margin; subcontrol-position: top left; 
            left: 12px; top: 0px; color: #165DFF; font-weight: bold; font-size: 14px; 
            padding: 4px 10px; background-color: #E8F3FF; border-radius: 6px; border: 1px solid #BFE0FF;
        }
        
        /* 表格样式 */
        QTableWidget { background-color: #FFFFFF; gridline-color: transparent; border: 1px solid #E5E6EB; border-radius: 8px; outline: none; }
        QTableWidget::item { border-bottom: 1px solid #F2F3F5; padding: 5px; }
        QTableView::item:hover { background-color: #F7F8FA; }
        QTableView::item:selected { background-color: #E8F3FF; color: #165DFF; }
        
        QHeaderView::section { background-color: #F7F8FA; padding: 12px; border: none; border-bottom: 2px solid #E5E6EB; font-weight: bold; color: #4E5969; text-align: left; }
        
        /* 进度条 */
        QProgressBar { border: none; border-radius: 4px; background-color: #E5E6EB; color: transparent; max-height: 8px; }
        QProgressBar::chunk { background-color: #00B42A; border-radius: 4px; }
        
        QSplitter::handle { background: transparent; width: 16px; }
        
        /* 扁平化滚动条 */
        QScrollBar:vertical { border: none; background: transparent; width: 8px; margin: 0px; }
        QScrollBar::handle:vertical { background: #C9CDD4; min-height: 30px; border-radius: 4px; }
    ...
    def init_ui(self):
        ...
        for i, text in enumerate(menu_items):
            ...
            btn.setProperty("cssClass", "nav-chip")  # 更改为 cssClass 避免保留字冲突
        ...
        top_bar = QHBoxLayout()
        btn_add_files = QPushButton("➕ 添加文件"); btn_add_files.setProperty("cssClass", "btn-primary")
        btn_add_folder = QPushButton("📁 添加文件夹"); btn_add_folder.setProperty("cssClass", "btn-primary")
        btn_clear = QPushButton("🗑️ 清空列表"); btn_clear.setProperty("cssClass", "btn-danger")
        btn_undo = QPushButton("↩️ 撤销上一步"); btn_undo.setProperty("cssClass", "btn-secondary") 
        ...
        for btn in [btn_add_files, btn_add_folder, btn_clear, btn_undo]: 
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
            top_bar.addWidget(btn)
        top_bar.addStretch()
        
        self.lbl_top_info = QLabel("当前导入: 0 个文件 | 选定待处理: 0 个")
        self.lbl_top_info.setStyleSheet("color: #4E5969; font-size: 13px; font-weight: bold;")
        ...
        empty_lbl.setStyleSheet("color: #C9CDD4; font-size: 15px; font-weight: bold; line-height: 1.5;")
        ...
        self.chk_test_mode.setStyleSheet("color: #4E5969; font-weight: bold;")
        
        self.progress = QProgressBar()
        self.lbl_status = QLabel("就绪")
        self.lbl_status.setStyleSheet("color: #86909C; margin-left: 10px;")
    ...
    def _create_subtitle(self, text):
        lbl = QLabel(text)
        lbl.setStyleSheet("color: #86909C; font-size: 12px; margin-bottom: 8px;")
    ...
    def create_media_panel(self):
        ...
        if not HAS_TINYTAG:
            ...
            lbl_warn.setStyleSheet("color: #F53F3F; font-size: 12px; font-weight: bold; margin-bottom: 5px;")
        ...
        lbl_vars.setStyleSheet("color: #4E5969; font-size: 12px; background: #E5E6EB; padding: 12px; border-radius: 8px; margin-top: 10px;")
    ...
    def create_flatten_panel(self):
        ...
        self.lbl_flatten_src = QLabel("等待选择..."); self.lbl_flatten_src.setStyleSheet("color: #86909C;")
        
        self.btn_select_flatten_dest = QPushButton("2. 选择提取目标地"); self.btn_select_flatten_dest.setProperty("cssClass", "btn-secondary")
        self.lbl_flatten_dest = QLabel("等待选择..."); self.lbl_flatten_dest.setStyleSheet("color: #86909C;")
    ...
    def create_security_log_panel(self):
        ...
        self.txt_logs.setStyleSheet("font-size: 12px; background: #FFFFFF; border: 1px solid #E5E6EB;")
    ...
    def select_dir(self, label):
        ...
        if d: label.setText(d); label.setStyleSheet("color: #165DFF; font-weight: bold;") 
    ...
    def get_current_rules(self):
        return {
            ...
            'seq_padding': self.spin_seq_pad.value(),
    ...
    def render_table(self):
        # 【修复核心：屏蔽信号】防止 setItem 触发 itemChanged 导致死循环递归
        ...
        for row, record in enumerate(self.file_records):
            ...
            item_path.setForeground(QColor("#C9CDD4"))
            self.table.setItem(row, 6, item_path)

        # 【修复核心：恢复信号】表格渲染完成后，恢复接收信号
        self.table.blockSignals(False)

        # 重新绑定事件监听，确保先解绑以免多次触发
        ...
        if conflict_count > 0:
            ...
            self.lbl_status.setStyleSheet("color: #F53F3F; font-weight: bold; margin-left:10px;")
        else:
            self.lbl_status.setText("规则校验通过，无冲突")
            self.lbl_status.setStyleSheet("color: #00B42A; margin-left:10px;")
