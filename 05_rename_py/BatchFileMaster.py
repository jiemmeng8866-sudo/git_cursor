...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QComboBox, QFileDialog, QMessageBox, QGridLayout, QButtonGroup, QSplitter,
...
# ================= 现代化自定义步进器 (替换丑陋的原生 QSpinBox) =================
class ModernStepper(QWidget):
    """现代质感数字输入器：取代原生带上下箭头的 QSpinBox"""
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("批量文件管理专家 V4.5 (莫兰迪胶囊标签极致版)")
    ...
    def apply_morandi_theme(self):
        """莫兰迪高级绿主题 QSS：修复了卡片截断，加入了胶囊按钮样式"""
        brand_color = "#6D8764" 
        bg_color = "#F5F6F4"    
        card_bg = "#FFFFFF"     
        ...
        QWidget {{ color: #2C332A; font-family: "Microsoft YaHei", "Segoe UI", sans-serif; font-size: 13px; }}
        ...
        .btn-primary {{ 
            background-color: {brand_color}; 
            color: #FFFFFF; border: none; border-radius: 8px; padding: 9px 18px; font-weight: bold;
        }}
        .btn-primary:hover {{ background-color: #5A7352; }}
        
        .btn-secondary {{ 
            background-color: #FFFFFF; color: #5C6658; border: 1px solid #DCE0D9; border-radius: 8px; padding: 9px 18px; font-weight: bold;
        }}
        .btn-secondary:hover {{ background-color: #F0F2ED; border-color: #C5CCC0; }}
        
        .btn-danger {{ 
            background-color: #FCF2F2; color: #C94A4A; border: 1px solid #F5D5D5; border-radius: 8px; padding: 9px 18px; font-weight: bold;
        }}
        .btn-danger:hover {{ background-color: #FAEOEO; border-color: #EBBBBB; }}
        
        .btn-execute {{ 
            background-color: #2C332A; color: #FFFFFF; border: none; font-size: 15px; border-radius: 12px; font-weight: bold;
        }}
        ...
        QGroupBox {{ 
            border: 1px solid #EAECE8; 
            border-radius: 16px; 
            margin-top: 26px; /* 顶部留出足够的绝对空间给标题 */
        ...
        QTableWidget {{ 
            background-color: {card_bg}; 
            gridline-color: transparent; border: none; border-radius: 16px; outline: none;
        }}
        ...
        QHeaderView::section {{ 
            background-color: #F0F2ED; padding: 10px 12px; border: none; border-bottom: 2px solid #EAECE8; font-weight: bold; color: #5C6658; text-align: left;
        }}
    ...
    def init_ui(self):
        ...
        for i, text in enumerate(menu_items):
            ...
            col = i % 2
            if i == 4: # 最后一个独占一行居中
                nav_layout.addWidget(btn, row, col, 1, 2)
            else:
    ...
    def get_current_rules(self):
        return {
            ...
            'seq_start': self.spin_seq_start.value(), # 已兼容 ModernStepper 的 value() 方法
